...
use MediaProperty\Publisher as ChildPublisher;
use MediaProperty\PublisherQuery;
...
abstract class Book implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\MediaItem\\Map\\BookTableMap';
    ...
    /**
     * Initializes internal state of MediaItem\Base\Book object.
     */
     ...
     * @return   \MediaItem\Book The current object (for fluent API support)
     */
    ...
    public function setYear($v)
    {
        ...
        if ($this->year !== $v) {
            $this->year = $v;
            $this->modifiedColumns[BookTableMap::YEAR] = true;
        }
    ...
    public function setCover($v)
    {
        ...
        if ($this->cover !== $v) {
            $this->cover = $v;
            $this->modifiedColumns[BookTableMap::COVER] = true;
        }
    ...
    public function setPublisherId($v)
    {
        ...
        if ($this->publisher_id !== $v) {
            $this->publisher_id = $v;
            $this->modifiedColumns[BookTableMap::PUBLISHER_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : BookTableMap::translateFieldName('Year', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : BookTableMap::translateFieldName('Cover', TableMap::TYPE_PHPNAME, $indexType)];
            $this->cover = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : BookTableMap::translateFieldName('PublisherId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->publisher_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : BookTableMap::translateFieldName('AuthorId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException("Error populating \MediaItem\Book object", 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(BookTableMap::YEAR)) {
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[3], $arr)) $this->setIsbn($arr[$keys[3]]);
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(BookTableMap::YEAR)) $criteria->add(BookTableMap::YEAR, $this->year);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \MediaItem\Book (or compatible) type.
     ...
     *
     * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return                 \MediaItem\Book Clone of current object.
     * @throws PropelException
     */

...
abstract class Author implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\MediaProperty\\Map\\AuthorTableMap';
    ...
    /**
     * Initializes internal state of MediaProperty\Base\Author object.
     */
     ...
     * @return   \MediaProperty\Author The current object (for fluent API support)
     */
    ...
    public function setDob($v)
    {
        ...
        if ($this->dob !== $v) {
            $this->dob = $v;
            $this->modifiedColumns[AuthorTableMap::DOB] = true;
        }
    ...
    public function setDod($v)
    {
        ...
        if ($this->dod !== $v) {
            $this->dod = $v;
            $this->modifiedColumns[AuthorTableMap::DOD] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : AuthorTableMap::translateFieldName('Dob', TableMap::TYPE_PHPNAME, $indexType)];
            $this->dob = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : AuthorTableMap::translateFieldName('Dod', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException("Error populating \MediaProperty\Author object", 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(AuthorTableMap::DOB)) {
            $modifiedColumns[':p' . $index++]  = 'DOB';
        }
        if ($this->isColumnModified(AuthorTableMap::DOD)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(AuthorTableMap::DOB)) $criteria->add(AuthorTableMap::DOB, $this->dob);
        if ($this->isColumnModified(AuthorTableMap::DOD)) $criteria->add(AuthorTableMap::DOD, $this->dod);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \MediaProperty\Author (or compatible) type.
     ...
     *
     * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return                 \MediaProperty\Author Clone of current object.
     * @throws PropelException
     */
    ...
    public function initBooks($overrideExisting = true)
    {
        ...
        $this->collBooks->setModel('\MediaItem\Book');
    }

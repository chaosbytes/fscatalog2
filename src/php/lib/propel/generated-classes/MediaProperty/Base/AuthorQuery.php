...
use \PDO;
use MediaItem\Book;
use MediaProperty\Author as ChildAuthor;
use MediaProperty\AuthorQuery as ChildAuthorQuery;
use MediaProperty\Map\AuthorTableMap;
 ...
 * @method     ChildAuthorQuery orderByDob($order = Criteria::ASC) Order by the dob column
 * @method     ChildAuthorQuery orderByDod($order = Criteria::ASC) Order by the dod column
 *
 ...
 * @method     ChildAuthorQuery groupByDob() Group by the dob column
 * @method     ChildAuthorQuery groupByDod() Group by the dod column
 *
 ...
 * @method     ChildAuthor findOneByDob(string $dob) Return the first ChildAuthor filtered by the dob column
 * @method     ChildAuthor findOneByDod(string $dod) Return the first ChildAuthor filtered by the dod column
 *
 ...
 * @method     array findByDob(string $dob) Return ChildAuthor objects filtered by the dob column
 * @method     array findByDod(string $dod) Return ChildAuthor objects filtered by the dod column
 *
 */
abstract class AuthorQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \MediaProperty\Base\AuthorQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'fscatalog2', $modelName = '\\MediaProperty\\Author', $modelAlias = null)
    {
    ...
    {
        if ($criteria instanceof \MediaProperty\AuthorQuery) {
            return $criteria;
        }
        $query = new \MediaProperty\AuthorQuery();
        if (null !== $modelAlias) {
    ...
    {
        $sql = 'SELECT ID, FIRST_NAME, LAST_NAME, DOB, DOD FROM author WHERE ID = :p0';
        try {
    ...
    public function filterByDob($dob = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(AuthorTableMap::DOB, $dob, $comparison);
    }
    ...
    public function filterByDod($dod = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(AuthorTableMap::DOD, $dod, $comparison);
    }

    /**
     * Filter the query by a related \MediaItem\Book object
     *
     * @param \MediaItem\Book|ObjectCollection $book  the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildAuthorQuery The current query, for fluid interface
     */
    ...
    {
        if ($book instanceof \MediaItem\Book) {
            return $this
            ...
            throw new PropelException('filterByBook() only accepts arguments of type \MediaItem\Book or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \MediaItem\BookQuery A secondary query class using the current class as primary query
     */
    public function useBookQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinBook($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Book', '\MediaItem\BookQuery');
    }
